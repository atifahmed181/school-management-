...
import { Fee, FeePayment, Student } from '../models';
...
export class FeeController {
  ...
  static async createFee(req: Request, res: Response) {
    try {
      ...
      const fee = await Fee.create({ name, description, amount });
  ...
  static async createPayment(req: Request, res: Response) {
    try {
      ...
      const payment = await FeePayment.create({ studentId, feeId, amountPaid, paymentDate, paymentMethod, remarks });
  ...
  static async getPayments(req: Request, res: Response) {
    try {
      const payments = await FeePayment.findAll({ include: [Fee, Student] });
      res.json({ payments });
    } catch (err) {
      res.status(500).json({ error: 'Internal server error', details: err.message });
    }
  ...
  static async getPaymentById(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const payment = await FeePayment.findByPk(id, { include: [Fee, Student] });
